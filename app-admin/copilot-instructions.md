...
1. **ALL COMPONENTS SHOULD USE `ChangeDectionStrategy.OnPush`**: Every component you generate **MUST** use `ChangeDetectionStrategy.OnPush`. The `@Component` decorator **MUST** include the property `changeDetection: ChangeDetectionStrategy.OnPush`.
...
2. **USE NATIVE CONTROL FLOW**: You **MUST** use built-in `@` syntax for all control flow in templates.
...
3. **CHECK YOUR OUTPUT WITH THE ANGULAR COMPILER AND FIX ERRORS**: After you complete the project generation, run the `ng build` command and observe the output to check for errors. Fix any errors you find.

4. **USE BROWSER NATIVE MODERN SCSS**: You **MUST** user built-in SCSS unless asked to use another styling library by the user.
