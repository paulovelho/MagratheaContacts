...
export class FormService {
	...
	public Build(fields:string[], required:string[]=[], translationArr?:string[]): FormGroup {
		let controls: any = {};
		let index = 0;
		fields.forEach((f: string) => {
			controls[f] = [''];
			if(translationArr?.length != null)
				this.translations[f] = translationArr[index] ?? f;
			index ++;
		});
	...
	public setObject(form: FormGroup, object: any, touch:boolean=false): FormGroup {
		const keys = Object.keys(form.controls!);
		keys.forEach(key => {
			form.controls[key].setValue(object[key]);
			if(touch && object[key]) form.controls[key].markAsDirty();
		});
		return form;
	}
